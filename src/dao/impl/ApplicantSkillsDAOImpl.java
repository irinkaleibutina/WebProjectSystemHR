...
import static dao.util.sql.ApplicantSkillsSQL.*;
    ...
    public void addSkills(int applicantId, Map<String, String> skills) throws DAOException {
        ...
        try (Connection connection = connectionPool.getConnection()) {
            try (PreparedStatement preparedStatement = connection
                    .prepareStatement(SEARCH_APPLICANT_SKILL)) {
                preparedStatement.setInt(1, applicantId);
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    if (resultSet.isBeforeFirst()) {
                        setApplicantSkills(applicantId, skills, connection, UPDATE_APPLICANT_SKILLS);
                    } else {
                        setApplicantSkills(applicantId, skills, connection, ADD_SKILL);
                    }
                }
            }
